...
class docView(ModelView):
    ...
    def read_api(self):
        ...
        kids = list(dict({"id": i.id, "name": i.name}) for i in item.kids)
        parents = list(dict({"id": i.id, "name": i.name}) for i in item.parents)
        return jsonify({
    ...
    def get_lib(self, libname):
        return self.render_template("lib.html", lib=libname)
